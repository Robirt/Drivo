...
{
    public LecturersService(UserManager<UserEntity> userManager)
    {
    ...
    public async Task<ActionResponse> CreateLecturerAsync(CreateUserRequest request)
    {
        ...
        if ((await UserManager.CreateAsync(new LecturerEntity(userName, request.Email, request.FirstName, request.LastName, request.BirthDate)) is var createResult && createResult.Succeeded == false))
        {
